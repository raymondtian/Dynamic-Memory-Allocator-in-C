...
int is_alloc(int index, unsigned char * free_block_list) {
...
//Recursive function to find and free allocated block
int free_allocated_block(int index, int current_heap_address, int max_heap_address,
int location, int min_size, unsigned char * free_block_list) {
    ...
    int is_alloc_value = is_alloc(index, ptr);
    ...
    while (is_alloc_value != 1) {
        ...
        value = free_allocated_block(previous_index, previous_heap_address, max_heap_address, location - 1, min_size, ptr);

        //if value returned from the function is -1 (ie: free block)
        if (value == -1) {
            return free_allocated_block(next_index, next_heap_address, max_heap_address, location - 1, min_size, ptr);;
        } else {
            return value;
        }
        is_alloc_value = 1;
...
void merge2(int index, int current_heap_address, int max_heap_address, 
int location, int allocation_size, int min_size, unsigned char * free_block_list) {
    ...
    if (ptr[index] == 4) {
        if (current_heap_address + current_size > max_heap_address) {
            if (current_heap_address <= max_heap_address) {
                if (ptr[next_index] == 1 && ptr[previous_index] == 1) {
                    ptr[index] = 1;
                    return;
                } else {
                    merge2(previous_index, previous_heap_address, max_heap_address, location - 1, allocation_size, min_size, ptr);
                    merge2(next_index, next_heap_address, max_heap_address, location - 1, allocation_size, min_size, ptr);
                }
            }
        }
    }
